(() => {
  ...
  function equals() {
    ...
    current = format(value); stored = null; operator = null; waiting = false; render();
  }
  ...
  function beep() {
    ...
    try {
      ...
      osc.frequency.value = 520; gain.gain.setValueAtTime(0.025, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.045);
      osc.connect(gain); gain.connect(ctx.destination); osc.start(); osc.stop(ctx.currentTime + 0.045);
    } catch (_) {}
  }
  ...
  function syncSettings() {
    ['sound','vibration','motion','compact'].forEach(k => { const el = $(k + 'Toggle'); if (el) el.checked = Boolean(settings[k]); });
    saveSettings();
  }
  ...
  document.addEventListener('keydown', event => {
    ...
    else if (key === 'Escape') { if ($('settingsPanel')?.classList.contains('open')) closeSettings(); else clearAll(); }
